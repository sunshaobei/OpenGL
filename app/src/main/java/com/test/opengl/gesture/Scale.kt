...
class Scale constructor(var glView: GLView) : Shape(), GLView.DispatchTouchListener {
    ...
    override fun onDrawFrame(gl: GL10) {
        Matrix.multiplyMM(outputMatrix, 0, mMVPMatrix, 0, scleMatrix, 0)
        draw()
    }
    ...
    override fun dispatchTouchEvent(event: MotionEvent?): Boolean {
        ...
        if (event != null) {
            val normalizedX = toOpenGLCoord(event.x, true)
            val normalizedY = toOpenGLCoord(event.y, false)
            when (event.actionMasked) {
                ...
                MotionEvent.ACTION_POINTER_DOWN -> {
                    isScale = true
                    var x1 = toOpenGLCoord(event.getX(1), true)
                    var y1 = toOpenGLCoord(event.getY(1), false)
                ...
                MotionEvent.ACTION_MOVE -> {
                    if (isScale) {
                        var x2 = toOpenGLCoord(event.getX(1), true)
                        var y2 = toOpenGLCoord(event.getY(1), false)
                        var dis_start2 = computeDis(normalizedX, x2, normalizedY, y2)
                        val d = dis_start2 / dis_start
                        var s: Float = d.toFloat()
                        Matrix.scaleM(scleMatrix, 0, s, s, 0.0f)
