...
//        GLES20.glEnable(GLES20.GL_ALPHA)
    ...
    override fun onSurfaceChanged(gl: GL10, width: Int, height: Int) {
        ...
        if (mBitmap == null) mBitmap = createBp(width, height)
        val w = mBitmap!!.width
        ...
        if (width > height) {
            if (sWH > sWidthHeight) {
                Matrix.frustumM(mProjectMatrix, 0, -sWidthHeight * sWH, sWidthHeight * sWH, -1f, 1f, 3f, 7f)
            } else {
                Matrix.frustumM(mProjectMatrix, 0, -sWidthHeight / sWH, sWidthHeight / sWH, -1f, 1f, 3f, 7f)
            }
        } else {
            if (sWH > sWidthHeight) {
                Matrix.frustumM(mProjectMatrix, 0, -1f, 1f, -1 / sWidthHeight * sWH, 1 / sWidthHeight * sWH, 3f, 7f)
            } else {
                Matrix.frustumM(mProjectMatrix, 0, -1f, 1f, -sWH / sWidthHeight, sWH / sWidthHeight, 3f, 7f)
            }
        }
        //设置相机位置
